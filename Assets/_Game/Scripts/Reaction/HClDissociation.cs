...
public class HClReaction : IReaction
{
    public void CheckForReaction(ReactionContext context, in IOnlyPushList<IReactingReaction> result)
    {
        if (!context.ContainsMolecule(Molecules.HydrochloricAcid))
            return;

        result.Push(new HClDissociationReacting());
    }
}

public class HClDissociationReacting : IReactingReaction
{
    public int GetPriority() => 100;

    public IReadOnlyList<Molecule> GetReactants() => new[]
    {
        Molecules.HydrochloricAcid
    };
    ...
    public int GetProductMolarRatio(Molecule product)
    {
        ...
        if (product == Molecules.Chloride) return 1;
        return 0;
    }

    public Dictionary<Molecule, int> GetOrders() => new()
    {
        { Molecules.HydrochloricAcid, 1 }
    };
    ...
    public string GetId() => "hcl_dissociation";
